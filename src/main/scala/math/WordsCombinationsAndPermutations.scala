...
object WordsCombinationsAndPermutations {

  def chooseAmongst(k: Int, n: Int, words: Array[String]): Array[String] = ???

  def permute(list: Array[String]): Array[Array[String]] = ???

  def stats: Unit = {
    val f12 = factorial(12)
    val f2048divf2036 = multiply(Range(2037, 2049).toList) //2048!/2036! = 2048 * ... * 2037, range is exclusive

    val combinations = f2048divf2036 / f12
    println(s"combinations: $combinations")

    println(s"permutations: $f12")

    println(s"$f2048divf2036 possibilities")
  ...
  def multiply(list: List[Int]): BigInt = list.foldLeft(BigInt(1)){ case (acc,i) => BigInt(i) * acc }

  val WordList = Array("abandon", "ability", "able", "about", "above",
