...
//  println(lsort_solution(List(List('a, 'b, 'c), List('d, 'e), List('f, 'g, 'h), List('d, 'e), List('i, 'j, 'k, 'l), List('m, 'n), List('o))))
  ...
  def lsort_solution[A](list: List[List[A]]): List[List[A]] = {
    list.sortWith(_.length <= _.length )
  }

  def lsortFreq[A](list: List[List[A]]): List[List[A]] = {

    list.sortWith((left, right) => list.count(ls => ls.length == left.length) <= list.count(ls => ls.length == right.length))
  }

//  def lsortFreq_solution[A](ls: List[List[A]]): List[List[A]] = {
